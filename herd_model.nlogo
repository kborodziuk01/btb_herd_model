extensions [
  time
  csv] ; model uses the time extension for scheduling of the events
...
to go
  ...
  [feed
    ;show "IT'S 7AM GIRLS, !FEDDING TIME"
  ...
  [feed
    ;show "IT'S 6PM GIRLS, !FEDDING TIME"
  ]
  ...
  ask cows with [status = "Healthy" and dry? = false]
  [
...
to cow_move
  ...
  ask cows with [dry? = false] [move-to one-of neighbors with [ pcolor != red]]
...
to cow_food_stuff
  ...
  ifelse hunger >= hunger_treshold
  [
   let p 0

  ifelse side = "left"
    [ set p one-of l_food_patches]
    [ set p one-of r_food_patches]

    let np [neighbors] of p

    move-to one-of np with [pcolor != red]
